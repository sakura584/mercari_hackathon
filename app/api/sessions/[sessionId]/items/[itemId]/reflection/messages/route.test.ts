...
describe("POST reflection messages", () => {
  ...
  it("persists Gemini's ask_question function call", async () => {
    ...
    const res = await POST(request("大切です"), { params: Promise.resolve({ sessionId: session.id, itemId: item.id }) });
    expect((await res.json()).action).toBe("ask");
    expect((await getReflectionState(session.id, item.id))?.reasonsToKeep).toEqual(["思い出"]);
  });
  ...
  it("forces complete_reflection after the maximum turns", async () => {
    ...
    for (let turn = 0; turn < 3; turn += 1) {
      ...
      await POST(request("回答"), { params: Promise.resolve({ sessionId: session.id, itemId: item.id }) });
    }
    generateContentMock.mockResolvedValueOnce({ functionCalls: [{ name: "complete_reflection", args: { reflection: "整理できました。", summary: {} } }] });
    await POST(request("最後の回答"), { params: Promise.resolve({ sessionId: session.id, itemId: item.id }) });
